			...
			if (e.getSource() == Setting_Button) {
				if(Setting_Button.getText() == "Setting") {
					try {
						...
						int src = Integer.parseInt(srcAd);
						int dst = Integer.parseInt(dstAd);

						if(!(1023<src && src<=0xFFFF) || !(1023<dst && dst<=0xFFFF)) {
							ChattingArea.setText("주소설정오류: 1024이상 65535이하의 수만 사용할 수 있습니다.\n");
						}else {
						...
						ChattingArea.setText("주소설정오류: 주소에는 숫자만을 입력해야합니다.\n");
					}
					...
					((ChatAppLayer) m_LayerMgr.GetLayer("Chat")).SetEnetSrcAddress(0);
					((ChatAppLayer) m_LayerMgr.GetLayer("Chat")).SetEnetDstAddress(0);
