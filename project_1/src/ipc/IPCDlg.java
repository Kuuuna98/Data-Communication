		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource() == Chat_send_Button) {
				if(Setting_Button.getText() == "Reset") {
					String ltext = ChattingWrite.getText();
					ChattingWrite.setText("");

					String ex = ChattingArea.getText();
					ChattingArea.setText(ex+"[SEND]: "+ltext+"\n");

					byte[] sbchange = ltext.getBytes();
					((ChatAppLayer) m_LayerMgr.GetLayer("Chat")).Send(sbchange,ltext.length());

				}else {
					//주소값이없으면“주소설정오류” MessageDialog를띄운다.
					JOptionPane.showMessageDialog(null, "주소설정오류","오류메세지",JOptionPane.ERROR_MESSAGE);
				}
			}
		}
	}
